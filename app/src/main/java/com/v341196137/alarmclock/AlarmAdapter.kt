...
class AlarmAdapter(private val alarm_list: LinkedList<AlarmData>) : RecyclerView.Adapter<AlarmAdapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        ...
        val alarmItem = LayoutInflater.from(parent.context).inflate(R.layout.alarm_item,
        parent, false) // most confusing line must go understand it afterwards
        return ViewHolder(alarmItem)
    }
    // over and over again to update items with data and scrolling data
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val currentAlarm = alarm_list[position]
        holder.timeTextView.text = (currentAlarm.alarm_time)
    ...
    class ViewHolder(alarm_item: View) : RecyclerView.ViewHolder(alarm_item){ // will contain instance of AlarmData --> alarm_item.xml
        ...
        // alarm_switch : Switch
        // Cache view holder
        val timeTextView : TextView = alarm_item.time_text_view
        //val alarmSwitch : Switch = alarm_item.alarm_switch
